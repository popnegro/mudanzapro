...
export default function QuoteCalculator({ 
  ...
  // Services State
  const [selectedServices, setSelectedServices] = useState<string[]>(['mudanza_premium']);
  ...
  const validateEmail = (emailVal: string) => {
    ...
    if (lower.endsWith('@gamil.com') || lower.endsWith('@gmial.com') || lower.endsWith('@gmeil.com')) {
  ...
  useEffect(() => {
    ...
    if (scheduledDate) {
      ...
      if (isWeekend && isEndOrStartOfMonth) {
        ...
        dateMultiplier = 0.90; // Weekday, mid-month promotion
      }
    }
    ...
    const baseMultiplier = ((origDept?.baseRateMultiplier || 1.0) + (destDept?.baseRateMultiplier || 1.0)) / 2;
  ...
  const handleNextStep = () => {
    ...
    if (step === 1) {
      ...
      if (!scheduledDate) {
        stepErrors.push('Por favor, indique la fecha estimada de su mudanza para verificar la tarifa y disponibilidad.');
      }
    } else if (step === 2) {
      if (Object.keys(selectedFurniture).length === 0) {
        stepErrors.push('Por favor, agregue al menos un mueble u objeto para cotizar.');
      }
    }
  ...
  const handleSubmitQuote = (e: React.FormEvent) => {
    ...
    if (nameErr || emailErr || phoneErr || dateErr) {
      ...
      setErrors(['Por favor, corrija los campos marcados con errores antes de enviar la cotización.']);
      return;
    }
  ...
  const getNameInputClass = () => {
    const base = "w-full bg-white border rounded-xl px-4 py-2.5 text-xs text-gray-800 focus:outline-none focus:ring-2 font-semibold transition-all duration-150";
    if (fieldErrors.customerName) {
      return `${base} border-red-400 focus:ring-red-500 bg-red-50/10`;
    }
    if (customerName.trim().length >= 5 && customerName.trim().includes(' ') && !/\d/.test(customerName)) {
  ...
  const getEmailInputClass = () => {
    ...
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (email.trim().length > 0 && emailRegex.test(email.trim()) && !email.trim().toLowerCase().endsWith('.con') && !email.trim().toLowerCase().includes('gamil') && !email.trim().toLowerCase().includes('gmial')) {
      return `${base} border-emerald-400 focus:ring-emerald-500 bg-emerald-50/10`;
    }
  ...
  const getPhoneInputClass = () => {
    ...
    if (phone.trim().length > 0 && cleanPhone.length >= 8 && cleanPhone.length <= 15) {
      return `${base} border-emerald-400 focus:ring-emerald-500 bg-emerald-50/10`;
    }
  ...
  const getDateInputClass = () => {
    const base = "w-full border rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:ring-2 font-bold transition-all duration-150";
  ...
  return (
    <div id="calculator-section" className="bg-white py-16 px-4 sm:px-6 lg:px-8 border-b border-gray-100">
      <div className="max-w-4xl mx-auto">
        <div className="text-center space-y-3 mb-10">
          ...
          <p className="text-sm text-gray-500 max-w-xl mx-auto">
            Configura los detalles de tu mudanza en Mendoza y obtén una estimación de precio transparente al instante.
          </p>
        </div>
        ...
        {success ? (
          <div className="bg-emerald-50 border border-emerald-100 p-8 sm:p-12 rounded-3xl text-center space-y-6 shadow-xs animate-fade-in">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-extrabold text-gray-900">¡Presupuesto Solicitado con Éxito!</h3>
              <p className="text-sm text-gray-600 max-w-lg mx-auto">
                Hola <strong className="text-gray-900">{customerName}</strong>, hemos registrado tu solicitud para el{' '}
                <strong className="text-gray-900">{new Date(scheduledDate + 'T00:00:00').toLocaleDateString('es-AR', { dateStyle: 'long' })}</strong>.
              </p>
              <div 
                itemScope 
              ...
              >
                ...
                <meta itemProp="validThrough" content={new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]} />
                <meta itemProp="url" content={`https://${activeBrand.domain}/calculadora`} />
                <span itemProp="seller" itemScope itemType="https://schema.org/MovingCompany">
            ...
            <p className="text-xs text-gray-500 max-w-md mx-auto">
              Te hemos enviado un correo a <strong className="text-gray-700">{email}</strong>. Un asesor técnico de{' '}
              {activeBrand.name} se comunicará contigo al <strong className="text-gray-700">{phone}</strong> por WhatsApp en los próximos 15 minutos para validar la lista y agendar.
            </p>
          ...
          <div className="bg-slate-50 rounded-3xl border border-gray-100 p-6 sm:p-8 shadow-sm">
            ...
            <div className="space-y-4 mb-8 pb-6 border-b border-gray-200/60">
              <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-wider text-gray-500">
                ...
                <span className="text-amber-600">Paso {step} de 4 ({step * 25}%)</span>
              </div>
              ...
              <div className="flex items-center justify-between gap-1 text-[10px] font-bold overflow-x-auto whitespace-nowrap pb-1">
                <button type="button" onClick={() => setStep(1)} className={`flex items-center gap-1.5 cursor-pointer shrink-0 ${step >= 1 ? 'text-amber-600' : 'text-gray-400'}`}>
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${step >= 1 ? 'bg-amber-600 text-white' : 'bg-gray-200 text-gray-700'}`}>1</span>
                  Ruta e Inmuebles
                </button>
                <div className="flex-1 min-w-4 h-0.5 bg-gray-200" />
                <button type="button" onClick={() => setStep(2)} className={`flex items-center gap-1.5 cursor-pointer shrink-0 ${step >= 2 ? 'text-amber-600' : 'text-gray-400'}`}>
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${step >= 2 ? 'bg-amber-600 text-white' : 'bg-gray-200 text-gray-700'}`}>2</span>
                  Muebles / Cajas
                </button>
                <div className="flex-1 min-w-4 h-0.5 bg-gray-200" />
                <button type="button" onClick={() => setStep(3)} className={`flex items-center gap-1.5 cursor-pointer shrink-0 ${step >= 3 ? 'text-amber-600' : 'text-gray-400'}`}>
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${step >= 3 ? 'bg-amber-600 text-white' : 'bg-gray-200 text-gray-700'}`}>3</span>
                  Servicios Extra
                </button>
                <div className="flex-1 min-w-4 h-0.5 bg-gray-200" />
                <button type="button" onClick={() => setStep(4)} className={`flex items-center gap-1.5 cursor-pointer shrink-0 ${step >= 4 ? 'text-amber-600' : 'text-gray-400'}`}>
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${step >= 4 ? 'bg-amber-600 text-white' : 'bg-gray-200 text-gray-700'}`}>4</span>
                  Confirmación
                </button>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-5 rounded-2xl border border-gray-100 space-y-4">
                  <h4 className="text-xs font-bold text-gray-800 tracking-wide uppercase flex items-center gap-1.5">
                    <Calendar className="w-4 h-4 text-amber-500" /> Fecha Planificada y Disponibilidad
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-extrabold text-gray-700 block">FECHA ESTIMADA DE MUDANZA</label>
                      <input
                        type="date"
                        required
                      ...
                      <p className="text-[10px] text-gray-400">
                        Selecciona tu fecha para calcular dinámicamente recargos por alta demanda o promociones.
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col justify-center">
                      {scheduledDate ? (
                        (() => {
                          ...
                          const dayOfMonth = dateObj.getDate();
                          const isWeekend = dayOfWeek === 0 || dayOfWeek === 5 || dayOfWeek === 6;
                          const isPeakMonth = dayOfMonth >= 27 || dayOfMonth <= 5;
                          
                          let statusColor = "text-emerald-600 bg-emerald-50 border-emerald-100";
                          let statusText = "🟢 Tarifa Promocional - Mitad de Mes";
                          let detailText = "Excelente fecha. Descuento del 10% aplicado sobre la tarifa base de traslado.";
                          
                          if (isWeekend && isPeakMonth) {
                            statusColor = "text-rose-600 bg-rose-50 border-rose-100";
                            statusText = "🔴 Demanda Crítica (Fin de Mes + Fin de Semana)";
                            detailText = "Cupos limitados. Se aplica un 25% de recargo por alta demanda operativa.";
                          } else if (isWeekend) {
                            statusColor = "text-amber-600 bg-amber-50 border-amber-100";
                            statusText = "🟡 Demanda Alta (Fin de Semana)";
                            detailText = "Se aplica un 15% de recargo de fin de semana. Reserva con anticipación.";
                          } else if (isPeakMonth) {
                            statusColor = "text-amber-600 bg-amber-50 border-amber-100";
                            statusText = "🟡 Demanda Alta (Fin de Mes)";
                            detailText = "Se aplica un 15% de recargo por recambio de alquileres. Quedan pocos camiones.";
                          }

                          return (
                            <div className={`p-4 rounded-xl border text-xs space-y-1 ${statusColor}`}>
                              <span className="font-extrabold block">{statusText}</span>
                              <p className="leading-relaxed font-medium">{detailText}</p>
                              <p className="text-[10px] opacity-80 pt-1 font-semibold">
                                Día seleccionado: {dateObj.toLocaleDateString('es-AR', { weekday: 'long', day: 'numeric', month: 'long' })}
                              </p>
                            </div>
                          );
                        })()
                        ...
                        <div className="p-4 rounded-xl border border-dashed border-gray-200 text-xs text-gray-400 flex flex-col items-center justify-center text-center py-6">
                          ...
                          <span className="font-semibold">Selecciona una fecha</span>
                          <span className="text-[10px]">Para habilitar promociones y validar la disponibilidad de camiones.</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5 pt-1">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-wider block">Sugerencias de Fechas</span>
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        ...
                        shortcuts.push({ label: 'Mañana', date: tom.toISOString().split('T')[0], icon: '⚡' });
                        
                        // Shortcut 2: Next Tuesday (off-peak usually)
                        const nextTue = new Date();
                        const currentDay = nextTue.getDay();
                        const distToTue = (currentDay <= 2) ? (2 - currentDay + 7) : (2 - currentDay + 14);
                        nextTue.setDate(nextTue.getDate() + (distToTue % 7 === 0 ? 7 : distToTue % 7));
                        shortcuts.push({ label: 'Próximo Martes (Promo)', date: nextTue.toISOString().split('T')[0], icon: '🟢' });

                        // Shortcut 3: Next Saturday (weekend)
                        const nextSat = new Date();
                        const distToSat = (6 - nextSat.getDay() + 7) % 7;
                        nextSat.setDate(nextSat.getDate() + (distToSat === 0 ? 7 : distToSat));
                        shortcuts.push({ label: 'Próximo Sábado', date: nextSat.toISOString().split('T')[0], icon: '🟡' });
                ...
                <div className="bg-white p-5 rounded-2xl border border-gray-100 space-y-4">
                  <h4 className="text-xs font-bold text-gray-800 tracking-wide uppercase flex items-center gap-1">
                    <Info className="w-3.5 h-3.5 text-amber-500" /> Complejidad de Accesos (Escaleras/Pisos)
                  </h4>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {/* Origin Access */}
                    <div className="space-y-3.5">
                      <p className="text-xs font-bold text-gray-900 border-b border-gray-100 pb-1.5">Origen (Donde cargamos)</p>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500 flex-1">¿Tiene ascensor de carga?</span>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500 flex-1">Piso de la vivienda:</span>
                        <div className="flex items-center gap-1.5">
                          <button
                            type="button"
                            onClick={() => setFloorOrigin(prev => Math.max(0, prev - 1))}
                            className="p-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 transition"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="w-8 text-center text-xs font-bold text-gray-900">{floorOrigin === 0 ? 'PB' : floorOrigin}</span>
                          <button
                            type="button"
                            onClick={() => setFloorOrigin(prev => Math.min(25, prev + 1))}
                            className="p-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 transition"
                          >
                    ...
                    {/* Destination Access */}
                    <div className="space-y-3.5">
                      <p className="text-xs font-bold text-gray-900 border-b border-gray-100 pb-1.5">Destino (Donde descargamos)</p>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500 flex-1">¿Tiene ascensor de carga?</span>
                        <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200 text-xs">
                          <button
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            type="button"
                            onClick={() => setFloorDest(prev => Math.max(0, prev - 1))}
                            className="p-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 transition"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="w-8 text-center text-xs font-bold text-gray-900">{floorDest === 0 ? 'PB' : floorDest}</span>
                          <button
                            type="button"
                            onClick={() => setFloorDest(prev => Math.min(25, prev + 1))}
                            className="p-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 transition"
                          >
                            <Plus className="w-3 h-3" />
                          </button>
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-extrabold text-gray-800 mb-2">CANTIDAD DE BIENES A TRASLADAR</h3>
                  <p className="text-xs text-gray-400 mb-4">
                    Haz clic en los botones para indicar qué muebles y cajas vas a mudar. Esto calcula el volumen necesario de camión.
                  </p>
                </div>
                ...
                <div className="bg-amber-50 border border-amber-200/60 p-3.5 rounded-2xl text-xs text-amber-900 font-bold flex items-start gap-2.5 shadow-3xs animate-fade-in">
                  ...
                  <div>
                    <span className="block text-amber-800 text-xs font-black">💡 ¿Prefieres agilizar tu cotización?</span>
                    <span className="text-[11px] text-amber-700 font-medium leading-relaxed mt-0.5 block">
                      Dejamos precargada una lista residencial estándar de Mendoza para que no pierdas tiempo. Si no deseas detallar unidad por unidad, puedes presionar <strong>"Siguiente"</strong> ahora mismo; nuestro equipo adaptará la lista exacta contigo por llamada telefónica o WhatsApp.
                    </span>
                  </div>
                </div>
                ...
                {['living', 'dormitorio', 'cocina', 'otros'].map(cat => {
                  const itemsInCat = FURNITURE_ITEMS.filter(f => f.category === cat);
                  return (
                    <div key={cat} className="space-y-3 bg-white p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-3xs">
                      <h4 className="text-xs font-black text-gray-800 uppercase tracking-wider border-b border-gray-100 pb-2">
                        {cat === 'living' ? '🛋️ Living y Comedor' : cat === 'dormitorio' ? '🛏️ Dormitorio' : cat === 'cocina' ? '🍳 Cocina y Lavadero' : '📦 Cajas y Varios'}
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        {itemsInCat.map(item => {
                          const count = selectedFurniture[item.id] || 0;
                          return (
                            <div key={item.id} className="flex justify-between items-center text-xs p-2 rounded-xl hover:bg-slate-50 border border-transparent hover:border-gray-100 transition">
                              ...
                              <div className="flex items-center gap-2.5">
                                {count > 0 && (
                                  <>
                                    <button
                                      type="button"
                                      onClick={() => adjustFurnitureCount(item.id, -1)}
                                    ...
                                    <span className="w-4 text-center font-black text-gray-800">{count}</span>
                                  </>
                                )}
                                <button
                                  type="button"
                ...
                <div className="p-4 rounded-xl bg-amber-50/50 border border-amber-100 flex justify-between items-center text-xs text-amber-800">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-extrabold block">Espacio total de carga estimado: {totalVolumePoints} unidades</span>
                      <span className="text-[10px] text-gray-500">Mudarás una cantidad de bultos clasificada como: <strong>{moveSize.toUpperCase()}</strong></span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-between pt-2">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 px-6 py-3 bg-gradient-to-r ${activeBrand.primaryColor} text-white font-bold text-xs rounded-xl hover:opacity-95 shadow-sm transition cursor-pointer`}
                  >
                    Elegir Servicios Adicionales <ArrowRight className="w-4 h-4" />
                  </button>
                </div>
              </div>
            )}
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-sm text-gray-900">{srv.name}</span>
                            {isSelected && <span className="bg-amber-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md uppercase">SELECCIONADO</span>}
                          </div>
                          ...
                          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1 pt-1 text-[10px] text-gray-400">
                            {srv.features.slice(0, 2).map((feat, i) => (
                              <li key={i} className="flex items-center gap-1">✓ {feat}</li>
                            ))}
                          </ul>
                        </div>
            ...
            {step === 4 && (
              <form onSubmit={handleSubmitQuote} className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-extrabold text-gray-500 flex items-center justify-between">
                      ...
                      {customerName.trim().length >= 5 && !fieldErrors.customerName && (
                    ...
                    {fieldErrors.customerName ? (
                      ...
                      <p className="text-[10px] text-emerald-600 font-semibold flex items-center gap-1 mt-1 animate-fade-in">
                        <Check className="w-3.5 h-3.5 shrink-0" /> Nombre registrado y habilitado para cobertura de seguro.
                      </p>
                    ) : (
                      <p className="text-[9px] text-gray-400">Ingrese su nombre y apellido completo.</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-extrabold text-gray-500 flex items-center justify-between">
                      <span className="flex items-center gap-1">
                        <Calendar className="w-3.5 h-3.5 text-amber-500" /> FECHA ESTIMADA DE MUDANZA
                      </span>
                    ...
                    {fieldErrors.scheduledDate ? (
                      <p className="text-[10px] text-red-500 font-semibold flex items-center gap-1.5 mt-1 animate-fade-in">
                        <AlertCircle className="w-3.5 h-3.5 shrink-0" /> {fieldErrors.scheduledDate}
                      </p>
                    ) : (
                      <p className="text-[9px] text-amber-600 font-semibold flex items-center gap-1 mt-1">
                        <Check className="w-3 h-3" /> Fecha seleccionada y cotizada en el Paso 1.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {fieldErrors.email ? (
                      ...
                      <p className="text-[10px] text-emerald-600 font-semibold flex items-center gap-1 mt-1 animate-fade-in">
                        <Check className="w-3.5 h-3.5 shrink-0" /> Email válido. Te enviaremos copia del presupuesto por este medio.
                      </p>
                    ) : (
                      <p className="text-[9px] text-gray-400">Recibirá una copia formal en este correo.</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-extrabold text-gray-500 flex items-center justify-between">
                      <span className="flex items-center gap-1">
                        <Phone className="w-3.5 h-3.5 text-gray-400" /> TELÉFONO DE CONTACTO / WHATSAPP
                      </span>
                    ...
                    {fieldErrors.phone ? (
                      ...
                      <p className="text-[10px] text-emerald-600 font-semibold flex items-center gap-1 mt-1 animate-fade-in">
                        <Check className="w-3.5 h-3.5 shrink-0" /> WhatsApp válido. Un asesor de Mendoza te enviará la confirmación.
                      </p>
                    ) : (
                      <p className="text-[9px] text-gray-400">Ejemplo de formato local: 261 5555555</p>
                    )}
                  </div>
                </div>
                ...
                {/* Notes */}
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-gray-500">INDICACIONES ADICIONALES (OPCIONAL)</label>
                  <textarea
                    rows={2}
