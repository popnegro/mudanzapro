...
export default function QuoteCalculator({
  ...
  const calculateRoute = async () => {
    ...
    try {
      const response = await fetch("/api/route", {
        ...
        body: JSON.stringify({
          origin: {
            ...
            ...(originCoords ? { latLng: originCoords } : {}),
          },
          destination: {
            address: destinationAddress,
            ...(destinationCoords ? { latLng: destinationCoords } : {}),
          },
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(payload?.code || "ROUTES_API_ERROR");
      }
      ...
      setRouteError(
        "No pudimos calcular el recorrido ahora. Podés continuar y solicitar el presupuesto igualmente.",
      );
    } finally {
    ...
    return (
      <section className="bg-[#FAF9F5] px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-xl rounded-3xl border border-[#06434A]/10 bg-white p-8 shadow-sm sm:p-10">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#07BE8A]/10 text-[#009966]">
            <Check className="h-6 w-6" />
          </div>
          <h2 className="mt-5 text-center text-2xl font-bold tracking-tight text-[#06434A]">
            Mudanzas Miranda puede evaluar tu traslado
          </h2>
          <p className="mt-3 text-center text-sm leading-6 text-slate-600">
            Ya tenemos los datos básicos del recorrido. El siguiente paso es solicitar un presupuesto con las condiciones reales de tu mudanza.
          </p>
          <div className="mt-6 rounded-2xl bg-[#FAF9F5] p-5">
            ...
            {routeEstimate && (
              <div className="mt-4 flex flex-wrap gap-3 border-t border-slate-200 pt-4 text-xs text-slate-600">
                <span className="inline-flex items-center gap-1.5"><Route className="h-3.5 w-3.5 text-[#009966]" />{routeEstimate.distanceText}</span>
                {routeEstimate.durationText && <span className="inline-flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5 text-[#009966]" />{routeEstimate.durationText}</span>}
              </div>
            )}
          </div>
          <div className="mt-5 rounded-2xl border border-[#06434A]/10 p-5">
            ...
            <button type="button" className="mt-4 min-h-11 w-full rounded-xl bg-[#07BE8A] px-5 text-sm font-bold text-[#06434A] transition hover:bg-[#009966] hover:text-white">
              Solicitar presupuesto a Mudanzas Miranda
            </button>
          </div>
          <button type="button" onClick={() => { setSuccess(false); setStep(1); }} className="mt-5 min-h-11 w-full rounded-xl border border-slate-200 px-5 text-sm font-semibold text-[#06434A]">
            Modificar datos
          </button>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-7">
          <div className="mb-4 flex items-center justify-between text-xs font-semibold text-slate-500">
            ...
            <span>{step === 1 ? "Trayecto" : step === 2 ? "Tamaño" : "Contacto"}</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-slate-200">
            <div className="h-full rounded-full bg-[#07BE8A] transition-all" style={{ width: `${(step / 3) * 100}%` }} />
          </div>
        </div>
        ...
        <form onSubmit={submit} className="rounded-3xl border border-[#06434A]/10 bg-white p-5 shadow-sm sm:p-8">
          {step === 1 && (
            <div className="space-y-6">
              <div>
                <h1 className="text-2xl font-bold tracking-tight text-[#06434A]">¿De dónde a dónde?</h1>
                <p className="mt-1 text-sm text-slate-500">Usamos el recorrido para conocer la distancia real del traslado.</p>
              </div>
              <div className="space-y-4">
                <AddressAutocomplete value={originAddress} onChange={(value) => { setOriginAddress(value); setOriginCoords(null); setRouteEstimate(null); }} onSelectAddress={(address, departmentId, lat, lng) => handleAddress("origin", address, departmentId, lat, lng)} placeholder="Dirección de origen" label="Origen" brandId={activeBrand.id} />
                <AddressAutocomplete value={destinationAddress} onChange={(value) => { setDestinationAddress(value); setDestinationCoords(null); setRouteEstimate(null); }} onSelectAddress={(address, departmentId, lat, lng) => handleAddress("destination", address, departmentId, lat, lng)} placeholder="Dirección de destino" label="Destino" brandId={activeBrand.id} />
                <div>
                  <label htmlFor="scheduled-date" className="mb-2 block text-sm font-semibold text-slate-700">Fecha estimada</label>
                  <input id="scheduled-date" type="date" value={scheduledDate} min={new Date().toISOString().split("T")[0]} onChange={(event) => setScheduledDate(event.target.value)} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-800 outline-none focus:border-[#07BE8A] focus:ring-2 focus:ring-[#07BE8A]/15" />
                </div>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold tracking-tight text-[#06434A]">¿Qué tamaño tiene?</h2>
                <p className="mt-1 text-sm text-slate-500">Elegí una aproximación. No usamos este dato para inventar un precio.</p>
              </div>
              <div className="grid gap-3">
                {MOVE_OPTIONS.map((option) => {
                  const selected = moveSize === option.id;
                  return (
                    <button key={option.id} type="button" onClick={() => setMoveSize(option.id)} className={`min-h-16 rounded-2xl border p-4 text-left transition ${selected ? "border-[#07BE8A] bg-[#07BE8A]/5 ring-2 ring-[#07BE8A]/10" : "border-slate-200 bg-white hover:border-slate-300"}`}>
                      <span className="block text-sm font-bold text-[#06434A]">{option.label}</span>
              ...
              <div className="rounded-2xl bg-[#FAF9F5] p-5">
                <p className="text-xs font-bold uppercase tracking-wider text-[#009966]">Datos del recorrido</p>
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <div className="flex items-center gap-2 text-sm text-slate-700"><MapPin className="h-4 w-4 text-[#009966]" />{routeEstimate?.distanceText || "Distancia no disponible"}</div>
                  <div className="flex items-center gap-2 text-sm text-slate-700"><Clock3 className="h-4 w-4 text-[#009966]" />{routeEstimate?.durationText || "Duración no disponible"}</div>
                </div>
                {routeLoading && <p className="mt-3 text-xs text-slate-500">Calculando recorrido...</p>}
                {routeError && <p className="mt-3 text-xs leading-5 text-slate-500">{routeError}</p>}
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold tracking-tight text-[#06434A]">¿Cómo te contactamos?</h2>
                <p className="mt-1 text-sm text-slate-500">Estos datos se usan para solicitar el presupuesto a Mudanzas Miranda.</p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div><label htmlFor="customer-phone" className="mb-2 block text-sm font-semibold text-slate-700">Teléfono</label><input id="customer-phone" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Tu teléfono" autoComplete="tel" inputMode="tel" className="min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#07BE8A] focus:ring-2 focus:ring-[#07BE8A]/15" /></div>
              ...
              <div className="rounded-2xl bg-[#06434A] p-5 text-white">
                <p className="text-xs font-bold uppercase tracking-wider text-[#07BE8A]">Siguiente paso</p>
                <p className="mt-2 text-sm leading-6 text-white/80">Al enviar, tu solicitud queda preparada para pedir un presupuesto a Mudanzas Miranda. El precio final lo determina la empresa según las condiciones reales del traslado.</p>
              </div>
            </div>
          )}
