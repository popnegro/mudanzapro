...
import { MessageSquare, X, Send, Check, AlertCircle, Phone, ShieldCheck, Sparkles } from 'lucide-react';
...
export default function WhatsAppWidget({ activeBrand, viewMode }: WhatsAppWidgetProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      <AnimatePresence>
        {showTooltip && (
          ...
          >
            ...
            <span>¿Necesitás cotizar? Respondé 2 preguntas por WhatsApp 🚀</span>
            <button 
              onClick={(e) => {
                e.stopPropagation();
                setShowTooltip(false);
      ...
      <motion.button
        ...
        aria-label={isOpen ? "Cerrar ventana de contacto por WhatsApp" : "Abrir ventana de contacto por WhatsApp"}
      ...
      >
        {isOpen ? (
          ...
          <>
            <MessageSquare className="w-6 h-6 fill-current stroke-[2]" aria-hidden="true" />
            <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-rose-500 rounded-full border-2 border-white animate-ping" aria-hidden="true" />
            <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-rose-500 rounded-full border-2 border-white" aria-hidden="true" />
          </>
        )}
      </motion.button>
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="bg-emerald-600 text-white p-5 relative overflow-hidden shrink-0">
              <div className="absolute right-0 bottom-0 top-0 w-1/3 opacity-10 bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:12px_12px]" aria-hidden="true" />
              <div className="flex items-center gap-3 relative z-10">
                <div className="relative">
                  ...
                  <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-400 rounded-full border-2 border-emerald-600 animate-pulse" aria-hidden="true" />
                </div>
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-emerald-100 leading-none">WhatsApp Express</h4>
                  <p className="text-base font-extrabold leading-tight mt-1">Sofía - Soporte</p>
                  <p className="text-[10px] text-emerald-100 font-medium mt-0.5">En línea • Respuesta en menos de 5 min</p>
                </div>
              </div>
            </div>
 
            {/* Chat Body & Form */}
            <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[380px] overflow-y-auto bg-slate-50">
              ...
              <div className="bg-white rounded-2xl rounded-tl-none p-3.5 border border-gray-100 shadow-3xs text-xs text-gray-700 leading-relaxed font-semibold">
                ¡Hola! 👋 Bienvenido al portal de <span className="text-emerald-700 font-black">{activeBrand.name}</span>.
                <br />
                Decime tu nombre y contame brevemente qué necesitas trasladar (muebles, origen y destino). Te cotizo al instante por WhatsApp.
              </div>
 
              {/* Input Name */}
              <div className="space-y-1">
                <label htmlFor="wa-name-field" className="text-[9px] font-black text-gray-500 uppercase tracking-wider block">Tu Nombre</label>
                <div className="relative">
                  <input
                    id="wa-name-field"
                    type="text"
                  ...
                  {waName.trim().length >= 3 && !waErrors.name && (
                    <Check className="absolute right-3 top-3 w-4 h-4 text-emerald-500 stroke-[3]" aria-hidden="true" />
                  )}
                </div>
                {waErrors.name && (
                  <p className="text-[9px] text-red-500 font-bold flex items-center gap-1 animate-fade-in">
                    <AlertCircle className="w-3 h-3 shrink-0" aria-hidden="true" /> {waErrors.name}
                  </p>
                )}
              </div>
              ...
              {/* Input Message */}
              <div className="space-y-1">
                <label htmlFor="wa-msg-field" className="text-[9px] font-black text-gray-500 uppercase tracking-wider block">Tu Consulta</label>
                <textarea
                  id="wa-msg-field"
                  rows={2}
                  value={waMsg}
                ...
                {waErrors.msg && (
                  <p className="text-[9px] text-red-500 font-bold flex items-center gap-1 animate-fade-in">
                    <AlertCircle className="w-3 h-3 shrink-0" aria-hidden="true" /> {waErrors.msg}
                  </p>
                )}
                ...
                {(() => {
                  ...
                  if (!hint) return null;
                  return (
                    <div className={`p-2 rounded-lg text-[9px] mt-1 border leading-relaxed font-bold animate-fade-in ${
                      hint.type === 'success'
              ...
              >
                <Phone className="w-3.5 h-3.5 stroke-[2.5]" aria-hidden="true" />
