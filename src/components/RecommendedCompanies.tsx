...
export default function RecommendedCompanies({
  ...
  const allZones = useMemo(() => {
    ...
    RECOMMENDED_COMPANIES.forEach((c) =>
      c.zones.forEach((z) => zonesSet.add(z)),
    );
  ...
  const allSpecialties = useMemo(() => {
    ...
    RECOMMENDED_COMPANIES.forEach((c) => {
      ...
      if (
        c.specialties.some(
          (s) =>
            s.toLowerCase().includes("izamiento") ||
            s.toLowerCase().includes("izaje"),
        )
      )
        specsSet.add("Izajes");
      if (c.specialties.some((s) => s.toLowerCase().includes("piano")))
        specsSet.add("Traslado de Pianos");
      if (c.specialties.some((s) => s.toLowerCase().includes("larga")))
        specsSet.add("Larga Distancia");
      if (
        c.specialties.some(
          (s) =>
            s.toLowerCase().includes("hogar") ||
            s.toLowerCase().includes("residencial"),
        )
      )
        specsSet.add("Hogar");
      if (
        c.specialties.some(
          (s) =>
            s.toLowerCase().includes("económico") ||
            s.toLowerCase().includes("básico") ||
            s.toLowerCase().includes("express"),
        )
      )
        specsSet.add("Económico");
    });
  ...
  const getZoneForDeptId = (deptId: string): string => {
    if (["tunuyan", "tupungato", "san_carlos"].includes(deptId))
      return "Valle de Uco";
    if (["san_rafael", "general_alvear", "malargue"].includes(deptId))
      return "Zona Sur";
    return "Gran Mendoza";
  };
  ...
  const filteredCompanies = useMemo(() => {
    return RECOMMENDED_COMPANIES.filter((company) => {
      ...
      return (
        matchesSearch && matchesZone && matchesSpecialty && matchesFeatured
      );
    });
  }, [
    searchQuery,
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-gray-500 max-w-2xl mx-auto">
            Compara transportistas habilitados con reputación intachable en la
            provincia. Filtrados por zona de cobertura, especialidad de traslado
            y cotizaciones base verificadas.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div
                ...
                role="group"
                aria-label="Filtros de región de cobertura"
              >
                <button
            ...
            <div className="space-y-2">
              ...
              <div
                className="flex flex-wrap gap-1.5"
                role="group"
                aria-label="Filtros de especialidad de servicio"
              >
                <button
            ...
            {filteredCompanies.map((company) => (
              ...
              >
                ...
                <div className="p-6 sm:p-7 space-y-4 flex-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div
                      ...
                      aria-label={`Especialidades ofrecidas por ${company.name}`}
                    >
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <ul
                      ...
                      aria-label={`Garantías y seguridad de ${company.name}`}
                    >
                ...
                <div className="p-6 bg-slate-50/80 rounded-b-3xl border-t border-gray-100/80 flex flex-col gap-3">
                  ...
                  {(company.id === "emp-mendoza-mza" ||
                    company.id === "emp-miranda-mza") &&
                    onBrandSelect && (
                      <button
                        onClick={() => {
                          const targetBrand =
          ...
          <div className="text-center bg-slate-50 border border-dashed border-gray-200 py-12 px-6 rounded-3xl max-w-md mx-auto space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-gray-500 max-w-xs mx-auto">
                Prueba borrando tu búsqueda, seleccionando "Todas las zonas" o
                buscando una especialidad diferente.
              </p>
            </div>
            <button
              onClick={() => {
        ...
        <div className="bg-amber-50/50 border border-amber-100 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row gap-6 items-center justify-between">
          <div className="space-y-2">
            ...
            <p className="text-xs text-amber-800/80 leading-relaxed max-w-3xl">
              Nuestros prestadores deben certificar: Habilitación comercial
              local en Mendoza, seguros de carga vigentes provistos por
              aseguradoras registradas en la SSN, vehículos con inspección
              técnica aprobada y un índice superior a 4.5 estrellas en
              valoraciones reales. No cobramos comisiones por traslados,
              garantizando total transparencia.
            </p>
          </div>
