...
export default function Breadcrumbs({
  ...
  if (activeBrandId !== 'empresas') {
    items.push({
      label: activeBrandId === 'mendoza' ? 'Mudanzas Mendoza' : 'Mudanzas Miranda',
  ...
  return (
    <div id="dynamic-breadcrumbs-container" className="bg-white border-b border-gray-100 py-3 px-4 sm:px-6 lg:px-8">
      <nav 
      ...
      >
        ...
        <div className="hidden sm:flex items-center gap-1.5 bg-slate-50 border border-slate-200/50 rounded-full px-2.5 py-1 text-[10px] font-black uppercase text-gray-500 tracking-wider">
          ...
          <span>Nodo de Autoridad: {activeBrandId === 'empresas' ? 'Directorio Central' : `Socio Verificado (${activeBrand.name})`}</span>
        </div>
      </nav>
    </div>
  );
