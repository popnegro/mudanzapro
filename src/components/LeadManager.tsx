...
import { 
  FolderOpen, Calendar, DollarSign, Filter, Search, Phone, 
  Trash2, Mail, CheckCircle2, MessageSquare, Clock, ArrowUpRight 
} from 'lucide-react';
...
export default function LeadManager({ leads, onUpdateLeadStatus, onDeleteLead }: LeadManagerProps) {
  ...
  const formatDate = (isoString: string) => {
    try {
      ...
      return date.toLocaleDateString('es-AR', { dateStyle: 'medium' }) + ' ' + date.toLocaleTimeString('es-AR', { timeStyle: 'short' });
    } catch {
      return isoString;
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-gray-400 max-w-xl mx-auto">
            Simula cómo tu equipo recibe, filtra y responde a las solicitudes de cotizaciones del portal SEO en tiempo real. ¡Prueba agregando un lead en el cotizador!
          </p>
        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md ${
                              lead.brand === 'mendoza' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-sky-500/10 text-sky-400 border border-sky-500/20'
                            }`}>
                          ...
                          <p className="text-[10px] text-slate-500">Solicitado: {formatDate(lead.createdAt)}</p>
                        </div>

                        <div className="text-right space-y-1.5">
                          <p className="text-xs font-black text-amber-400">${lead.estimatedCost.toLocaleString('es-AR')}</p>
                          <span className={`inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full ${
                            ...
                            <span className={`w-1.5 h-1.5 rounded-full ${
                              lead.status === 'new' ? 'bg-emerald-400 animate-pulse' : lead.status === 'contacted' ? 'bg-amber-400' : 'bg-sky-400'
                            }`} />
            ...
            {selectedLead ? (
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-start gap-2 pb-4 border-b border-slate-700/60">
                  <div>
                    <span className="text-[9px] font-bold text-amber-400 uppercase tracking-wider block">ID SOLICITUD: {selectedLead.id}</span>
                    <h4 className="text-lg font-black text-white mt-1">{selectedLead.customerName}</h4>
                    <p className="text-[10px] text-slate-400 mt-0.5">{selectedLead.email}</p>
                  </div>
                  <button
                    onClick={() => {
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-700/40">
                    ...
                    <a href={`tel:${selectedLead.phone}`} className="font-bold text-white mt-1 hover:underline flex items-center gap-1">
                  ...
                  <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-700/40 flex flex-col justify-between">
                    <div>
                      <span className="text-[9px] text-slate-500 font-bold block">FECHA AGENDADA</span>
                    ...
                    {(() => {
                      const count = dateAvailability[selectedLead.scheduledDate] || 0;
                      if (count > 1) {
                        return (
                ...
                <div className="space-y-2">
                  <span className="text-[9px] text-slate-500 font-bold block">CAMBIAR ESTADO DE LA SOLICITUD</span>
                  <div className="grid grid-cols-3 gap-2 text-xs font-bold">
                    ...
                    <button
                      onClick={() => onUpdateLeadStatus(selectedLead.id, 'completed')}
                      className={`px-3 py-2 rounded-xl transition cursor-pointer border ${
                ...
                <div className="bg-slate-900 p-4 rounded-2xl border border-slate-700/50 space-y-2.5">
                  <div className="flex justify-between items-center text-[10px] text-slate-500 font-extrabold tracking-wide uppercase">
                    <span className="flex items-center gap-1 text-emerald-400"><MessageSquare className="w-3.5 h-3.5" /> WHATSAPP AUTOMÁTICO</span>
                  ...
                  >
                    <ArrowUpRight className="w-3.5 h-3.5" /> Enviar por WhatsApp Real
                  </a>
                </div>

                {selectedLead.notes && (
                  <div className="space-y-1">
                    <span className="text-[9px] text-slate-500 font-bold block">NOTAS INTERNAS / CLIENTE</span>
              ...
              <div className="space-y-6">
                <div className="flex flex-col items-center justify-center py-6 text-center space-y-3 text-slate-500 border-b border-slate-700/45">
                  ...
                  <div className="space-y-1">
                    <p className="text-xs font-bold text-slate-400">Ningún Lead Seleccionado</p>
                    <p className="text-[10px] text-slate-500 max-w-xs">
                      Selecciona un lead de la izquierda para ver el detalle y gestionar la comunicación comercial.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-extrabold text-amber-400 flex items-center gap-1.5 uppercase tracking-wide">
                    <Calendar className="w-4 h-4" /> Control de Disponibilidad Operativa
                  </h4>
                  <p className="text-[10px] text-slate-400 leading-normal">
                    Sincronización en tiempo real de la flota de camiones basada en las solicitudes agendadas de los usuarios en Mendoza.
                  </p>

                  <div className="bg-slate-900/60 rounded-2xl p-4 border border-slate-700/50 space-y-3">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">ESTADO DE RESERVAS DE FLOTA</span>
                    {Object.keys(dateAvailability).length === 0 ? (
                      <p className="text-[10px] text-slate-500 text-center py-4 italic">
                        No hay mudanzas agendadas por el momento. Toda la flota está libre.
                      </p>
                    ) : (
                      <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1">
                        {Object.entries(dateAvailability)
                          ...
                          .map(([dateStr, count]) => {
                            ...
                            return (
                              <div key={dateStr} className="flex justify-between items-center bg-slate-950 p-2.5 rounded-xl border border-slate-800 text-xs">
                                <div className="space-y-0.5 text-left">
                                  <p className="font-bold text-slate-200">
                                    {dateObj.toLocaleDateString('es-AR', { day: 'numeric', month: 'short', weekday: 'short' })}
                                  </p>
                  ...
                  <div className="p-3 bg-slate-900/30 rounded-xl border border-slate-800 text-[10px] text-slate-400 leading-relaxed">
                    💡 <strong>Consejo del Operador:</strong> Cuando un día tenga el estado <span className="text-rose-400 font-bold">⚠️ SATURADO</span>, contacta al cliente para ofrecerle un incentivo por cambio de fecha (por ej. mudar el día anterior con 10% de descuento) y optimizar la capacidad de carga.
                  </div>
                </div>
              </div>
            )}
